...
class AdaptersFactory private constructor() {
    companion object {
        fun createObjectAdapter(vararg pairs: Pair<Features.Feature, Boolean>): JsonOpenApiSchemaAdapter {
            ...
            val features = Features(pairs.toMap())
            val schemaExtractor = RecursiveSchemaDefinitionExtractor(features)
            val yamlAdapter = RecursiveYamlObjectAdapter(yamlMapper, features)
